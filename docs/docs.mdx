...
Scroll alignment is automatically derived from the resolved popover position:

- **`position: 'top'`** → scrolls element to the end of the viewport
- **`position: 'bottom'`** → scrolls element to the start of the viewport
- **`position: 'left'` / `'right'`** → scrolls element to the center
