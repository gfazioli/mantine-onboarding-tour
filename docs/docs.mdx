import { Button, Code, rem, Table } from '@mantine/core';
...
The type is generic: custom properties are type-safe via the `T` parameter (e.g., `OnboardingTourStep<{ price: number }>`). See the [Custom entry](#custom-entry) section for an example.
...
<Table striped highlightOnHover withTableBorder withColumnBorders>
  <Table.Thead>
    <Table.Tr>
      <Table.Th>Callback</Table.Th>
      <Table.Th>When it fires</Table.Th>
    </Table.Tr>
  </Table.Thead>
  <Table.Tbody>
    <Table.Tr>
      <Table.Td>
        <Code>onOnboardingTourStart</Code>
      </Table.Td>
      <Table.Td>The tour starts</Table.Td>
    </Table.Tr>
    <Table.Tr>
      <Table.Td>
        <Code>onOnboardingTourChange</Code>
      </Table.Td>
      <Table.Td>The active step changes</Table.Td>
    </Table.Tr>
    <Table.Tr>
      <Table.Td>
        <Code>onOnboardingTourComplete</Code>
      </Table.Td>
      <Table.Td>
    ...
    <Table.Tr>
      <Table.Td>
        <Code>onOnboardingTourSkip</Code>
      </Table.Td>
      <Table.Td>The user clicks the "Skip" button</Table.Td>
    </Table.Tr>
    <Table.Tr>
      <Table.Td>
        <Code>onOnboardingTourEnd</Code>
      </Table.Td>
      <Table.Td>
...
<Demo data={demos.focusRevealDisableInteraction} />
...
The `OnboardingTour` component supports responsive behavior for mobile devices. You can control the mobile experience using the following props:

- `responsive`: Enable/disable responsive behavior (default: `true`)
- `mobileBreakpoint`: Mantine breakpoint name (default: `'sm'`)
- `mobilePosition`: Position of the guide on mobile - `'top'` or `'bottom'` (default: `'bottom'`)
...
When responsive mode is enabled on mobile devices:

- **Smart Positioning**: The guide automatically positions itself at the top or bottom of the screen
- **Optimized Scrolling**: Elements are scrolled into view with appropriate alignment
- **Mobile-First Design**: The guide uses full-width layout with proper spacing
- **Configurable Breakpoints**: You can customize when responsive behavior kicks in

### Mobile Position Options

- **`bottom`** (default): Guide appears at the bottom of the screen
- **`top`**: Guide appears at the top of the screen

### Custom Breakpoints

You can use any Mantine breakpoint name (`'xs'`, `'sm'`, `'md'`, `'lg'`, `'xl'`). The breakpoint values are read from the Mantine theme.

```tsx
// Extra small devices only
mobileBreakpoint = 'xs';

// Small devices and below (default)
mobileBreakpoint = 'sm';
...
You can also configure responsive behavior per step using `focusRevealProps`:
...
const steps = [
  {
    id: 'step1',
    title: 'Desktop Step',
    content: 'This step uses standard positioning',
    focusRevealProps: {
      responsive: false, // Disable responsive for this step
    },
  },
  {
    id: 'step2',
    title: 'Mobile Step',
    content: 'This step uses mobile positioning',
    focusRevealProps: {
      responsive: true,
      mobilePosition: 'top', // Override global setting
    },
  },
];
```
