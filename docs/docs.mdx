...
- `mobileBreakpoint`: Media query for mobile detection (default: `'(max-width: 768px)'`)
...
You can use any valid CSS media query for mobile detection:

```tsx
// Tablet and below
mobileBreakpoint = '(max-width: 1024px)';

// Small mobile only
mobileBreakpoint = '(max-width: 480px)';

// Custom breakpoint
mobileBreakpoint = '(max-width: 900px) and (orientation: portrait)';
```
